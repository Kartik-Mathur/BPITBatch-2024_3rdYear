...
public:
	int solve(vector<int>& prices, int i, int transactions, bool isActiveTransaction,
		...
		if (!isActiveTransaction) {
			int op1 = -prices[i] + solve(prices, i + 1, transactions + 1, true, dp);
			int op2 = 0 + solve(prices, i + 1, transactions, isActiveTransaction, dp);
			return dp[i][transactions] = max(op1, op2);
		}
		else {
			int op1 = prices[i] + solve(prices, i + 1, transactions + 1, false, dp);
			int op2 = 0 + solve(prices, i + 1, transactions, isActiveTransaction, dp);
			return dp[i][transactions] = max(op1, op2);
		}
	}
